...
xf_SortRealParallel2(real *u, int n, enum xfe_Bool SerialFlag, int *pos, int nelemtot_glob, int *locorders, int nelemref)
{
  ...
  if (myRank == 0){
	  ...
	  totalDOF = tmptotDOF;
	  if (nref < nelemref && stopRefine == 0) xf_printf("WARNING: nref < nelemref before max DOF reached!!!!!\n");
	}
	else{nreftot = 0;}
	  // if (cstop != 0){
...
*/
   ...
   */
       ...
       if((fabs(Time+(iadapt+1)*Yu_adapt_time_size - EndTime) > MEPS))
       {
           ...
           if (stopRefine == 1) xf_printf("Max DOF has been reached\n");
